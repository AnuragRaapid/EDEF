...
def parse_ner_json(ner_string: str) -> List[Tuple[str, str]]:
    """Parse NER JSON string and return list of (entity, type) tuples."""
    try:
        # Clean the string (remove trailing whitespace/newlines)
        ner_string = ner_string.strip()
        data = json.loads(ner_string)
        if "ner" in data:
            return [(str(entity).lower().strip(), str(etype).lower().strip()) 
...
def exact_match(pred_entities: List[Tuple[str, str]], 
                gold_entities: List[Tuple[str, str]]) -> Tuple[int, int, int]:
    """
...
def relaxed_match(pred_entities: List[Tuple[str, str]], 
                  gold_entities: List[Tuple[str, str]],
                  word_margin: int = 2) -> Tuple[int, int, int]:
    """
...
def evaluate_per_type(predictions: List[List[Tuple[str, str]]], 
                      golds: List[List[Tuple[str, str]]],
                      word_margin: int = 2) -> Dict:
    ...
    for etype in sorted(all_types):
        ...
        per_type_exact[etype] = {"precision": p, "recall": r, "f1": f1, 
                                  "tp": exact_tp, "fp": exact_fp, "fn": exact_fn}
        
        p, r, f1 = calculate_metrics(relaxed_tp, relaxed_fp, relaxed_fn)
        per_type_relaxed[etype] = {"precision": p, "recall": r, "f1": f1,
...
def main():
    ...
    input_file = "/home/anurag/NER/Multi-task Finetuning/predictions_qwen_4B_full_new.json"
    ...
    print(f"  Precision:       {relaxed_precision:.4f} ({relaxed_precision*100:.2f}%)")
    ...
    print(f"{'Entity Type':<35} {'Precision':>10} {'Recall':>10} {'F1':>10} {'Support':>10}")
    print("-" * 75)
    ...
    for etype in sorted(exact_types.keys()):
        ...
        print(f"{etype:<35} {metrics['precision']:>10.4f} {metrics['recall']:>10.4f} "
